...
def get_ban_name(filename: str) -> str:
    # Trích xuất tên ban từ tên file (loại bỏ "ban_" và ".xlsx", thay "_" thành " ")
    ban_name = filename.replace('ban_', '').replace('.xlsx', '').replace('_', ' ').title()
    return ban_name

# Đọc danh sách email từ file Excel
filename = 'ban_ai_engineer.xlsx'  # Thay bằng tên file thực tế hoặc lấy từ danh sách file
data = pd.read_excel(filename)
ban_name = get_ban_name(filename)
...
for index, row in data.iterrows():
    ...
    personalized_message = plaintext.format(name=row['Họ và tên'], ban_name=ban_name)
